...
[options="header"]
|=======================
|parameter|keyword      |example
| repository | `#` followed by `URL_of_the_repository` | `#https://github.com/eclipse-che/che-server/tree/7.40.x`
| editor | `che-editor` | `?che-editor=eclipse/che-theia/latest`
| override devfile path | `devfilePath` | `?devfilePath=.che/my-devfile.yaml`
| do not use existing workspace | `new` | `?new` 
|=======================
